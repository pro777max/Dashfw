...
void app_main(void) {
    ...
    // 1. ???????? ????????? ?? GPIO23 (LEDC)
    ESP_LOGI(TAG, "Init Backlight on GPIO23");
    ...
    ESP_LOGI(TAG, "Backlight is ON");

    // 2. ?????????????? ???????
    ESP_LOGI(TAG, "Init Display...");
    // ?????????? disp_cfg ? max_transfer_sz ???????, ??? ??? ????? ???? ?????? ??? 
    // ? ?????????? ?????? BSP. ??????? bsp_display_start() ?????????? ????????? ?? ?????????.
    bsp_display_start();
    bsp_display_lock(0);
    ...
    // LVGL 9: ?????????? lv_obj_set_clickable ?????? ??????????? lv_obj_remove_flag
    ...
    // ?????????? 32 ?????, ??? ??? 34 ?? ????????? ???????? ? sdkconfig.
    // (???? ????? ?????? 34, ?????????: idf.py menuconfig -> Component config -> LVGL -> Fonts -> Enable Montserrat 34)
