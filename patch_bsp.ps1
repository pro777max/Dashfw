# Скрипт для принудительной правки BSP в managed_components
\ = "managed_components/espressif__esp32_p4_function_ev_board/esp32_p4_function_ev_board.c"
if (Test-Path \) {
    Write-Host "Patching BSP source code to force EK79007..."
    \#include <stdio.h>
#include <string.h>
...
void app_main(void) {
 ...
 = Get-Content \ -Raw
    ...
    # Находим блок выбора дисплея и комментируем ILI9881C, оставляя только EK79007
    # Это грубый метод, но он работает на 100%
    \ = \#include <stdio.h>
#include <string.h>
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
...
void app_main(void) {
 ...
 -replace '#elif CONFIG_BSP_LCD_TYPE_1280_800', '// DISABLED BY PATCH: #elif CONFIG_BSP_LCD_TYPE_1280_800'
    
    Set-Content -Path \ -Value \ -NoNewline
    Write-Host "BSP patched successfully!"
} else {
    Write-Host "BSP file not found yet. It will be created by idf.py."
}
